...
import pytesseract
import re
from config import RAW_FOLDER, CLEAN_TEXT_FOLDER, CHUNK_FOLDER
...
def save_chunks(text, rel_path):
    ...
    base_folder = os.path.join(CHUNK_FOLDER, rel_path)
    os.makedirs(base_folder, exist_ok=True)
    for i, chunk in enumerate(chunks):
        chunk_rel_path = os.path.join(base_folder, f"chunk_{i+1}.txt")
        save_text(chunk, chunk_rel_path, folder="")  # folder="" because path includes folder

# ---------- Process File ----------
def process_file(file_path, rel_path):
    if os.path.basename(file_path).startswith("~$"):
        return 0, 0  # skipped, chars extracted = 0
    ...
    return 1, len(text_content)  # files processed, characters extracted
...
def process_all():
    ...
    for file_path, rel_path in tqdm(all_files, desc="Processing documents"):
        processed, chars = process_file(file_path, rel_path)
        total_files += processed
        total_chars += chars

    print(f"\n[SUMMARY] Total files processed: {total_files}")
    print(f"[SUMMARY] Total characters extracted: {total_chars}")
